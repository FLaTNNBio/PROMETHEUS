...
PROFILE_DGP_VERSION = "prometheus_synthetic_care_profile_dgp_v2"
NO_NEW_PROFILE = "no_new_profile"
DEFAULT_DGP_STRESS_SETTINGS = {
    "outcome_noise_sd": 5.0,
    "shared_response_scale": 1.0,
    "profile_specific_scale": 1.0,
    "idiosyncratic_effect_noise_sd": 0.38,
    "assignment_logit_scale": None,
    "assignment_exploration": None,
}
...
def _validate_stress_settings(settings: Mapping[str, float] | None) -> dict:
    result = dict(DEFAULT_DGP_STRESS_SETTINGS)
    if settings:
        unknown = sorted(set(settings).difference(result))
        if unknown:
            raise ValueError(f"Unknown profile DGP stress settings: {unknown}")
        result.update(dict(settings))
    bounded = {
        "outcome_noise_sd": (0.01, 50.0),
        "shared_response_scale": (0.0, 4.0),
        "profile_specific_scale": (0.0, 4.0),
        "idiosyncratic_effect_noise_sd": (0.0, 4.0),
    }
    for name, (lower, upper) in bounded.items():
        ...
        result[name] = value
    if result["assignment_logit_scale"] is not None:
        value = float(result["assignment_logit_scale"])
        if not 0.1 <= value <= 8.0:
            raise ValueError("assignment_logit_scale must be in [0.1, 8.0]")
        result["assignment_logit_scale"] = value
    if result["assignment_exploration"] is not None:
        value = float(result["assignment_exploration"])
        if not 0.001 <= value <= 0.30:
            raise ValueError("assignment_exploration must be in [0.001, 0.30]")
        result["assignment_exploration"] = value
    return result


def _assignment_regime(
    scenario: str, stress_settings: Mapping[str, float]
) -> tuple[float, float]:
    scale, exploration = 1.0, 0.10
    if scenario == "strong_observed_confounding":
        scale, exploration = 2.8, 0.035
    elif scenario == "poor_overlap":
        scale, exploration = 5.0, 0.01
    if stress_settings["assignment_logit_scale"] is not None:
        scale = float(stress_settings["assignment_logit_scale"])
    if stress_settings["assignment_exploration"] is not None:
        exploration = float(stress_settings["assignment_exploration"])
    return float(scale), float(exploration)
...
def _profile_effects(
    ...
    scenario: str,
    seed: int,
    stress_settings: Mapping[str, float],
) -> tuple[
    ...
    shared *= float(stress_settings["shared_response_scale"])
    ...
    profile_specific *= float(stress_settings["profile_specific_scale"])
    base = np.asarray([2.4, 3.6, 4.5, 4.2, 5.0, 5.8], dtype=float)
    idiosyncratic = rng.normal(
        0.0,
        float(stress_settings["idiosyncratic_effect_noise_sd"]),
        (n, len(profiles)),
    )
...
def _assignment_probabilities(
    ...
    scale, exploration = _assignment_regime(scenario, stress_settings)
    logits = (
...
def generate_synthetic_profile_dgp(
    ...
    stress_settings: Mapping[str, float] | None = None,
    ...
    stress = _validate_stress_settings(stress_settings)
    ...
    (
        ...
        patients, catalog, scenario, int(effect_seed), stress
    )
    ...
    baseline_primary_raw = (
        ...
        + outcome_rng.normal(
            0.0, float(stress["outcome_noise_sd"]), len(patients)
        )
    )
    ...
    assignment_scale, assignment_exploration = _assignment_regime(scenario, stress)
    identification = {
        "consistency_by_construction": True,
        "conditional_exchangeability_given_learner_covariates": scenario != "hidden_confounding",
        "positivity_stressed": bool(
            scenario == "poor_overlap"
            or assignment_scale >= 3.0
            or assignment_exploration <= 0.03
        ),
        ...
        "overlap_regime": (
            "poor"
            if scenario == "poor_overlap"
            or assignment_scale >= 4.5
            or assignment_exploration <= 0.015
            else "moderately_stressed"
            if assignment_scale >= 2.5 or assignment_exploration <= 0.04
            else "standard"
        ),
    }
    metadata = {
        ...
        "realized_assignment_logit_scale": assignment_scale,
        "realized_assignment_exploration": assignment_exploration,
