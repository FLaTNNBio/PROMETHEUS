...
def build_ems_policy_inputs(
    ...
    # One shared monotone calibrator preserves the global ordering learned
    # across both escalation opportunities. The network remains an ordinal
    # ranker; this mapping is used only by the downstream constrained allocator.
    global_calibrator = IsotonicRegression(
        increasing=True,
        out_of_bounds="clip",
    )
    global_calibrator.fit(calibration_score, calibration_signal)
    calibrated_test = global_calibrator.predict(test_score)
    prometheus_increment = {}
    for tier in (1, 2):
        test_mask = test_opportunities.opportunity_tier.eq(tier).to_numpy()
        tier_values = pd.Series(
            calibrated_test[test_mask],
    ...
    calibrator_audit = {
        "mode": "global_shared_isotonic",
        "calibration_rows": int(len(calibration)),
        "score_min": float(calibration_score.min()),
        "score_max": float(calibration_score.max()),
        "fitted_thresholds": int(len(global_calibrator.X_thresholds_)),
        "preserves_cross_tier_score_ordering": True,
    }
...
def _safe_rank_correlation(score: np.ndarray, target: np.ndarray, *, method: str) -> float:
    score = np.asarray(score, dtype=float)
    target = np.asarray(target, dtype=float)
    if len(score) < 2 or np.std(score) <= 1e-12 or np.std(target) <= 1e-12:
        return float("nan")
    if method == "spearman":
...
def _weighted_pairwise_concordance(
    score: np.ndarray,
    truth: np.ndarray,
    *,
    seed: int,
    maximum_pairs: int = 50_000,
) -> float:
    """Deterministic evaluation-only concordance on randomly sampled pairs."""

    score = np.asarray(score, dtype=float)
    truth = np.asarray(truth, dtype=float)
    if len(score) != len(truth) or len(score) < 2:
        return float("nan")
    rng = np.random.default_rng(int(seed))
    count = min(int(maximum_pairs), max(1, len(score) * 12))
    left = rng.integers(0, len(score), size=count)
    right = rng.integers(0, len(score), size=count)
    valid = (left != right) & (truth[left] != truth[right])
    if not valid.any():
        return float("nan")
    score_gap = score[left[valid]] - score[right[valid]]
    truth_gap = truth[left[valid]] - truth[right[valid]]
    concordant = (score_gap * truth_gap > 0.0).astype(float)
    concordant[score_gap == 0.0] = 0.5
    return float(concordant.mean())


def _topk_overlap(score: np.ndarray, truth: np.ndarray, fraction: float) -> float:
    score = np.asarray(score, dtype=float)
    truth = np.asarray(truth, dtype=float)
    k = max(1, min(len(score), int(round(len(score) * float(fraction)))))
    predicted = set(np.argsort(-score, kind="mergesort")[:k].tolist())
    oracle = set(np.argsort(-truth, kind="mergesort")[:k].tolist())
    return float(len(predicted & oracle) / k)


def _ndcg(score: np.ndarray, truth: np.ndarray, fraction: float) -> float:
    score = np.asarray(score, dtype=float)
    truth = np.asarray(truth, dtype=float)
    k = max(1, min(len(score), int(round(len(score) * float(fraction)))))
    relevance = pd.Series(truth).rank(method="average", pct=True).to_numpy(float)
    predicted = np.argsort(-score, kind="mergesort")[:k]
    oracle = np.argsort(-relevance, kind="mergesort")[:k]
    discount = 1.0 / np.log2(np.arange(2, k + 2))
    denominator = float(np.sum(relevance[oracle] * discount))
    if denominator <= 0.0:
        return float("nan")
    return float(np.sum(relevance[predicted] * discount) / denominator)


def evaluate_ems_policy_score_diagnostics(
    inputs: EMSPolicyInputs,
    evaluation_only: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    """Evaluate frozen learner-safe allocator values against synthetic truth.

    This function must be called only after policy values and allocation decisions
    have been frozen.  It never changes a policy; it produces evaluation-only
    diagnostics that distinguish within-tier ranking, global cross-tier ranking and
    within-mission tier selection.
    """

    mission_ids = inputs.mission_metadata.sort_values("mission_id").mission_id.astype(str)
    truth = evaluation_only.set_index("mission_id").loc[mission_ids]
    nurse_truth = truth.true_increment_nurse_supported.to_numpy(float)
    medical_truth = nurse_truth + truth.true_increment_medicalized.to_numpy(float)

    detail_rows: list[pd.DataFrame] = []
    summary_rows: list[dict[str, Any]] = []
    for policy, group in inputs.allocator_values.groupby("policy", sort=False):
        aligned = group.set_index("mission_id").loc[mission_ids]
        nurse_score = aligned.nurse_value.to_numpy(float)
        medical_score = aligned.medicalized_value.to_numpy(float)
        detail_rows.append(pd.DataFrame({
            "mission_id": mission_ids.to_numpy(str),
            "policy": str(policy),
            "nurse_score": nurse_score,
            "medicalized_score": medical_score,
            "true_nurse_benefit": nurse_truth,
            "true_medicalized_total_benefit": medical_truth,
        }))

        for tier_name, score, target, seed_offset in (
            ("nurse_supported", nurse_score, nurse_truth, 11),
            ("medicalized_total", medical_score, medical_truth, 23),
            ("global_opportunities", np.concatenate([nurse_score, medical_score]),
             np.concatenate([nurse_truth, medical_truth]), 37),
        ):
            spearman = _safe_rank_correlation(score, target, method="spearman")
            kendall = _safe_rank_correlation(score, target, method="kendall")
            summary_rows.append({
                "policy": str(policy),
                "scope": tier_name,
                "rows": int(len(score)),
                ...
                "spearman": spearman,
                "kendall": kendall,
                "pairwise_concordance": _weighted_pairwise_concordance(
                    score, target, seed=91_000 + seed_offset
                ),
                "ndcg_at_10pct": _ndcg(score, target, 0.10),
                "ndcg_at_25pct": _ndcg(score, target, 0.25),
                "top10pct_overlap": _topk_overlap(score, target, 0.10),
                "top25pct_overlap": _topk_overlap(score, target, 0.25),
                "oracle_access_for_policy_construction": False,
                "oracle_access_for_evaluation": True,
            })

        predicted_tier = medical_score > nurse_score
        true_tier = medical_truth > nurse_truth
        summary_rows.append({
            "policy": str(policy),
            "scope": "within_mission_tier_choice",
            "rows": int(len(nurse_score)),
            "score_std": float(np.std(medical_score - nurse_score)),
            "spearman": _safe_rank_correlation(
                medical_score - nurse_score,
                medical_truth - nurse_truth,
                method="spearman",
            ),
            "kendall": _safe_rank_correlation(
                medical_score - nurse_score,
                medical_truth - nurse_truth,
                method="kendall",
            ),
            "pairwise_concordance": float(np.mean(predicted_tier == true_tier)),
            "ndcg_at_10pct": float("nan"),
            "ndcg_at_25pct": float("nan"),
            "top10pct_overlap": float("nan"),
            "top25pct_overlap": float("nan"),
            "oracle_access_for_policy_construction": False,
            "oracle_access_for_evaluation": True,
        })
    ...
    audit = {
        "status": "evaluation_only_after_policy_freeze",
        "oracle_inputs_used_for_policy_construction": False,
        "oracle_inputs_used_for_evaluation": True,
        "policies": int(summary.policy.nunique()),
        "missions": int(len(mission_ids)),
    }
