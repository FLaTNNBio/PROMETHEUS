"""Orchestrate the canonical PROMETHEUS experiment from one versioned config.

Reusable scientific components live in ``causal_population_ranking``. This module
owns their execution order, artifact freezes and the boundary that opens synthetic
truth only for post-freeze evaluation.
"""
...
def _validate_config(config: Mapping) -> None:
    ...
    if tuple(calibration_contract["candidate_methods"]) != CALIBRATION_METHODS:
        raise ValueError("The pipeline requires the frozen calibrator")
    if tuple(map(str, calibration_contract["fit_partitions"])) != (
        "nuisance_train", "validation"
    ):
        raise ValueError("Calibration must exclude rank-training and test outcomes")
    ...
    if support[
        "require_population_pooled_evidence_lower_bound_above_null"
    ] is not True:
        raise ValueError("Recommendation requires positive pooled causal evidence")
    if support["population_pooled_evidence_is_individual_interval"] is not False:
        raise ValueError("Pooled evidence cannot be an individual-effect interval")
    ...
    if reporting["oracle_budget_curve_comparison"] != (
        "omitted_primary_budget_oracle_retained"
    ):
        raise ValueError(
            "Only the exact primary-budget oracle is supported in Phase-9B reporting"
        )
    ...
    if tuple(map(str, config["recommendation"]["profile_evidence_splits"])) != (
        "nuisance_train", "validation"
    ):
        raise ValueError(
            "Profile evidence must pool cross-fitted nuisance-train and validation rows"
        )
    if config["recommendation"]["profile_evidence_mode"] != (
        "population_pooled_nonnull_guardrail"
    ):
        raise ValueError("Phase-9B requires the population non-null evidence guardrail")
...
def run_pipeline(
    ...
    _write_json(stability_contract_path, {
        "contract_version": "prometheus_ranker_stability_diagnostic_v9",
    ...
    experiment_contract = {
        "contract_version": "prometheus_phase9b_stabilized_experiment_contract_v9",
        "protocol_version": experiment_config["protocol_version"],
