...
def _smoke_config(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["constraint_sensitivity"]["budget_multipliers"] = [
        0.25, 1.00, 1.50,
    ]
    result["constraint_sensitivity"]["capacity_multipliers"] = [
        0.50, 1.00, 1.25,
    ]
    return result
...
def run_ems_case_study(
    ...
    *,
    ...
    frozen_constraint_sensitivity = freeze_ems_constraint_sensitivity(
        policy_inputs,
        config,
    )
    ...
    nonoracle_value = float(
        ((heldout_gap > 0.0) + 0.5 * (heldout_gap == 0.0)).mean()
    )
    nonoracle = pd.DataFrame([{
        "method": "direct_causal_ranking",
        "metric": "heldout_dr_pairwise_concordance",
        "value": nonoracle_value,
        "split": "test",
        "uses_oracle": False,
