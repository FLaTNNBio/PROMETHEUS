...
def _fit_validation_calibrator(
    ...
    *,
    ...
    target = supervision.loc[
        supervision.split.astype(str).eq(VALIDATION_SPLIT)
    ...
    validation = target.merge(
        primary.loc[primary.split.astype(str).eq(VALIDATION_SPLIT)].drop(columns="split"),
    ...
    *,
    ...
    for profile, group in validation.groupby("care_profile_id", sort=True):
        ...
        rows.append({
            ...
            "profile_positive_validation_evidence": bool(
                count >= int(minimum_rows) and lower_bound > float(null_days)
            ),
    ...
    return pd.DataFrame(rows, columns=[
        ...
        "profile_positive_validation_evidence", "profile_evidence_oracle_used",
    ])
...
def calibrate_and_recommend_profiles(
    ...
    *,
    ...
    audit = {
        ...
        "primary_variant": str(primary_variant),
        "calibration_fit_partition": VALIDATION_SPLIT,
        ...
        "profile_validation_evidence_fit_partition": VALIDATION_SPLIT,
