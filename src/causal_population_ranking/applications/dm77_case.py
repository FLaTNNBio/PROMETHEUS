...
def sample_pairs(
    opportunities: pd.DataFrame,
    signal_columns: tuple[str, ...],
    *, split: str, maximum_pairs: int, minimum_gap: float, seed: int,
) -> pd.DataFrame:
    subset = opportunities.loc[opportunities.split.eq(split)].reset_index(drop=True)
    signal = subset[list(signal_columns)].mean(axis=1).to_numpy(float)
    rng = np.random.default_rng(seed)
    accepted: dict[tuple[int, int], tuple[int, int, float]] = {}
    for _ in range(40):
        left = rng.integers(0, len(subset), max(4096, maximum_pairs * 3))
        right = rng.integers(0, len(subset), len(left))
        gap = signal[left] - signal[right]
        valid = (
            subset.patient_id.to_numpy()[left] != subset.patient_id.to_numpy()[right]
        ) & (np.abs(gap) >= minimum_gap)
        for a, b, g in zip(left[valid], right[valid], gap[valid]):
            high, low = (int(a), int(b)) if g > 0 else (int(b), int(a))
            accepted.setdefault((high, low), (high, low, float(min(abs(g), 10.0))))
            if len(accepted) >= maximum_pairs:
                break
        if len(accepted) >= maximum_pairs:
            break
    frame = pd.DataFrame(accepted.values(), columns=["high_index", "low_index", "weight"])
    if frame.empty:
        raise ValueError(f"Could not sample DM77 pairs for {split}")
    frame["weight"] = frame.weight / frame.weight.mean()
    return frame
