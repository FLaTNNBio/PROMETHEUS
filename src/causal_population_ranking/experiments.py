...
def _learning_stack(
    ...
    *,
    ...
    dgp = generate_synthetic_profile_dgp(
        ...
        **{name: int(seeds[name]) for name in DATA_SEED_COMPONENTS if name != "population_seed"},
    )
    supervision_settings = copy.deepcopy(dict(config["causal_supervision"]))
    if causal_supervision_override:
        supervision_settings.update(copy.deepcopy(dict(causal_supervision_override)))
...
def build_candidate_declaration(
    ...
    *,
    ...
    return {
        "declaration_version": "prometheus_stabilized_locked_candidate_v7",
